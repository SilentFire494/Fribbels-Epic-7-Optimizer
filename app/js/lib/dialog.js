...
module.exports = {
    ...
    changeArtifact: (level) => {
        ...
        let html = '';
        const i18nAtk = i18next.t("atk");
        const i18nHp = i18next.t("hp");
        const i18nDef = i18next.t("def");

        for (let i = 30; i >= 0; i--) {
            const stats = Artifact.getStats(name, i);

            let statDisplay = [];
            if (stats.attack !== undefined) {
                statDisplay.push(`${stats.attack.toFixed(1)} ${i18nAtk}`);
            }
            if (stats.health !== undefined) {
                statDisplay.push(`${stats.health.toFixed(1)} ${i18nHp}`);
            }
            if (stats.defense !== undefined) {
                statDisplay.push(`${stats.defense.toFixed(1)} ${i18nDef}`);
            }

            html += `<option value="${i}">${i} - (${statDisplay.join(', ')})</option>`;
        }

        // Update the select element
        const $select = $("select[id='editArtifactLevel']");
...
function getArtifactEnhanceHtml(hero) {
    ...
    if (artifactName && artifactName !== "None") {
        ...
        if (artifactLevel && artifactLevel !== "None") {
            ...
            for (let i = 30; i >= 0; i--) {
                const stats = Artifact.getStats(artifactName, i);

                const statParts = [];
                if (stats.attack !== undefined) {
                    statParts.push(`${stats.attack.toFixed(1)} ${i18nAtk}`);
                }
                if (stats.health !== undefined) {
                    statParts.push(`${stats.health.toFixed(1)} ${i18nHp}`);
                }
                if (stats.defense !== undefined) {
                    statParts.push(`${stats.defense.toFixed(1)} ${i18nDef}`);
                }

                const selected = artifactLevel == i ? "selected" : "";
                html += `<option value="${i}" ${selected}>${i} - (${statParts.join(', ')})</option>`;
            }
        }
    }

...
public class StatCalculator {
    ...
    public void setBaseValues(final HeroStats base, final Hero hero) {
        ...
        bonusBaseAtk = base.getAtk() + base.getAtk() * (hero.getBonusAtkPercent() + hero.getAeiAtkPercent()) / 100f + hero.getBonusAtk() + hero.getAeiAtk();
        bonusBaseHp = base.getHp() + base.getHp() * (hero.getBonusHpPercent() + hero.getAeiHpPercent()) / 100f + hero.getBonusHp() + hero.getAeiHp();
        bonusBaseDef = base.getDef() + base.getDef() * (hero.getBonusDefPercent() + hero.getAeiDefPercent()) / 100f + hero.getBonusDef() + hero.getAeiDef();
    ...
    public HeroStats addAccumulatorArrsToHero(final HeroStats base, final float[][] accs, final int[] sets, final Hero hero, final int upgrades, final int conversions, final int alreadyEquipped, final int priority) {
        ...
        final float cr =         (base.getCr()  + accs0[6]+accs1[6]+accs2[6]+accs3[6]+accs4[6]+accs5[6] + (sets[4] > 1 ? sets[4] / 2 * 12 : 0) + hero.getBonusCr() + hero.getAeiCr());
        final int cd =   (int) (base.getCd()    + accs0[7]+accs1[7]+accs2[7]+accs3[7]+accs4[7]+accs5[7] + (sets[6] > 3 ? 60 : 0) + hero.getBonusCd() + hero.getAeiCd());
        final int eff =  (int) (base.getEff()   + accs0[8]+accs1[8]+accs2[8]+accs3[8]+accs4[8]+accs5[8] + (sets[5] > 1 ? sets[5] / 2 * 20 : 0) + hero.getBonusEff() + hero.getAeiEff());
        final int res =  (int) (base.getRes()   + accs0[9]+accs1[9]+accs2[9]+accs3[9]+accs4[9]+accs5[9] + (sets[9] > 1 ? sets[9] / 2 * 20 : 0) + hero.getBonusRes() + hero.getAeiRes());
        final int spd =  (int) (base.getSpd()   + accs0[10]+accs1[10]+accs2[10]+accs3[10]+accs4[10]+accs5[10] + (sets[3] > 3 ? speedSetBonus : 0) + (sets[14] > 3 ? revengeSetBonus : 0) + hero.getBonusSpeed() + hero.getAeiSpeed());
        ...
        final float bsHp = (hp - base.getHp() - hero.getArtifactHealth() - (sets[0] > 1 ? sets[0] / 2 * hpSetBonus : 0) + (sets[17] > 1 ? sets[17] / 2 * hpSetBonus/2 : 0)) / base.getHp() * 100;
        final float bsAtk = (atk - base.getAtk() - hero.getArtifactAttack() - (sets[2] > 1 ? sets[2] / 4 * atkSetBonus : 0)) / base.getAtk() * 100;
        ...
        final float bsSpd = (spd - base.getSpd() - (sets[3] > 3 ? speedSetBonus : 0) - (sets[14] > 3 ? revengeSetBonus : 0));
        ...
        return new HeroStats((int)atk, (int)hp, (int)def, (int) cr, cd, eff, res, 0, spd, cp, ehp, hpps, ehpps,
                ...
                base.getBonusStats(), null, null, null, null, null, null, null);
    }
